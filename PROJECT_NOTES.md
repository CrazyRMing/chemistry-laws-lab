...
- [x] **質量資訊搬移（舊儀表板 → 左側圖例）**：兩頁面頂部的質量儀表板（mass-board）全面刪除，改以左側面板下方的 HTML 圖例框呈現，名稱與數值換行（名稱：/ 換行 / H=…, O=…）。
- [x] **倍比定律步驟10右側圖改動**：步驟10右側圖與步驟9相同（顯示橫向比較線、紅綠括號），並移除橘色2.0與紫色1.0的文字，綠色括號在步驟10啟動時執行向下移動動畫。
- [x] **定比定律步驟6表格優化**：欄位標題分為兩行呈現並加入單位 `(g)`（如「氫質量 / wH (g)」，配合 `FONT_SMALL`），數字改用黑體 sans-serif 字型，儲存格內移除 `g` 單位，完全避免排版重疊。
- [x] **小蘇打名稱統一**：全站「加熱小蘇打水」一律改為「小蘇打分解水」（含左側圖例、燒杯標籤、步驟標題、Step 6 表格）。
- [x] **定比定律步驟7文字上下對齊**：普魯斯特名言改為 `textAlign = 'left'`，並使用全形空格縮排第2、3行，使三行文字左邊對齊。
- [x] **步驟5文字修改**：「它們在化學性質上完全一樣」改為「它們的組成元素重量比完全一樣」。
- [x] **倍比定律左側步驟6~9質量說明**：在步驟6~9左側微觀面板底部顯示「H = 2.5 g, O = 20.0 g」與「H = 1.5 g, O = 24.0 g」（淡入，對應各化合物欄位）。
