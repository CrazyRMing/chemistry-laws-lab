...
- [x] **倍比定律解析頁移除預設選取引導 (multiple_proportions_explain.js)**：移除了代數與幾何引導第一步（步驟1：請選擇欲固定的元素）中，原本將「固定 X 元素」預設設為 `active`（黑底白字）所產生的視覺暗示。現在兩顆按鈕皆預設為白底，且各自擁有高亮色的邊框（X元素為藍色，Y元素為紫色），對稱且無任何引導性，直到學生自行點擊選擇。
