...
- [x] **Header「挑戰」等頁碼超模修正 (style.css)**：修復了當頁碼方框（`.soil-page-num`）文字長度大於 2 或為全形中文字（如「挑戰」）時，文字因固定寬度而溢出方框邊界的 Bug。將其設定重構為自適應的 `min-width: 2.25rem` 並追加 `padding: 0 0.5rem` 與 `white-space: nowrap`，既不影響原先數字的方正外觀，又提供中文方框伸縮空間。
- [x] **全站網頁 RWD 視窗大小自適應強化 (style.css)**：
  - 新增針對 `@media (max-width: 768px)` 的媒體查詢，當視窗寬度不足時，Header 自動由橫向排版切換為縱向靠左排列，並允許右側按鈕區域自動換行（`flex-wrap: wrap`），避免按鈕與標題文字重疊。
  - 新增針對 `@media (max-width: 480px)` 的極小螢幕優化，自動縮小 `body` 及 `.app-container` 的內邊距 (padding) 與標題字型大小，確保在手機等窄屏下頁面完美適應，絕無水平溢出捲軸。
- [x] **倍比定律解析頁移除預設選取引導 (multiple_proportions_explain.js)**：
  - **第一步移除預設選取**：移除了第一步（請選擇欲固定的元素）中，原本將「固定 X 元素」預設設為 active（黑底白字）的視覺暗示。現在兩顆按鈕皆預設為白底，且各自擁有高亮色的邊框（X 元素為藍色，Y 元素為紫色），對稱且無任何引導性，直到學生自行點擊。
  - **第二步移除預設選取與按鈕啟用控制**：移除了第二步中對質量的預設值推薦，且在選取質量前將確認按鈕設為 disabled，Canvas 亦在點選後才開始繪線。
- [x] **倍比定律解析頁新增課本傳統詳解折疊卡片 (multiple_proportions_explain.html)**：在學測挑戰題卡片下方，新增了一個精美的折疊手風琴卡片（📖 觀看傳統課本詳解寫法）。該卡片採用截圖同款的精美桃紅色調（`#db2777`），完整還原了傳統表格、假設 $XY_n$ 與 $2 : 6 = 1 : n \Rightarrow n = 3$ 的推導過程。透過這個傳統詳解與下方「步驟化雙重表徵引導」的並列與對比，能極佳地凸顯出引導式學習在降低認知負荷、實現幾何具象化上的教學優勢。
- [x] **傳統詳解避嫌改寫以防版權爭議 (multiple_proportions_explain.html)**：將原圖截圖的解答文字「假設第二化合物為 XYn，固定 X 元素質量」與「則 Y 質量比 = Y 原子數比」等容易引發版權爭議的特定字句，改寫為「解析：設化合物 II 的化學式為 XYn，將兩化合物中 X 元素的質量調整至相同基準」與「此時，兩者 Y 元素的質量比例即等於其分子式中的原子個數比例」，科學計算邏輯與數值完全一致，但文字字句完全重構以防版權重合。
- [x] **GitHub Pages 公開部署配置**：
