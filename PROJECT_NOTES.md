...
- [x] **質量資訊與圖例排列優化**：兩頁面頂部的 mass-board 全面刪除。定比定律下方圖例框高度設為相同（112px，垂直置中對齊），左側改為 grid 兩欄排列，使「酒精燃燒水」位於「酸鹼中和水」右側；倍比定律左側圖例框改為垂直上下排列，與右側框大小對齊。
- [x] **倍比定律步驟10右側圖及括號動畫**：步驟10顯示與步驟9相同的比較結構，移除橘色 2.0 與紫色 1.0 文字，綠色括號在步驟 10 啟動時些微向下移動，但保持向上彎曲方向。
- [x] **定比定律步驟6表格與右側說明**：表格欄位標題分兩行並加入單位 (g)，數字改用黑體 sans-serif 且垂直置中，移除格內 `g` 單位。右側斜率說明文字取消斜體，改為黑體。
...
- [x] **定比與倍比法律文字套色**：定比定律步驟7卷軸文字中「化合物」、「質量比」、「為定值」標記為橘色；倍比定律步驟10卷軸中「固定甲元素的質量」標記為藍色，「簡單」標記為紅色，「整數比」標記為綠色。
