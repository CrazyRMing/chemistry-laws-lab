...
- [x] **倍比定律解析頁移除預設選取引導 (multiple_proportions_explain.js)**：
  ...
  - **第二步移除預設選取與按鈕啟用控制**：移除了第二步（設定固定後元素的目標質量）中，對質量的預設賦值推薦（如 `tempMass = 9.34` 或 `6.00`），現在進入第二步時所有質量按鈕皆預設不選取。同時，下方的「確認進行計算/推導」按鈕在未選取任何質量前會呈現 `disabled`（灰色停用狀態），直到學生手動點選某個質量按鈕後，確認按鈕才會動態解鎖（轉為 active），並同時在幾何 Canvas 上繪製輔助線，達成了完全無預設誘導的探究式教學流程。
