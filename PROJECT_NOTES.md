...
- [x] **質量資訊與圖例排列優化**：兩頁面頂部的質量儀表板（mass-board）全面刪除，左側 HTML 圖例框改為垂直上下排列（單行不折行）並與右側框大小對齊。
- [x] **倍比定律步驟10右側圖及括號動畫**：步驟10右側圖與步驟9相同，移除橘色 2.0 與紫色 1.0 文字，綠色括號在步驟 10 啟動時些微向下移動，但保持向上彎曲方向（`alignTop = true`）。
...
- [x] **倍比定律步驟6~9質量說明**：左面板上方不顯示 H/O 重量，改在左面板下方以對應的顏色（化合物 I 為橘色，化合物 II 為紫色）顯示，右側 legend 中的化合物 I 與 II 移除重量顯示。
